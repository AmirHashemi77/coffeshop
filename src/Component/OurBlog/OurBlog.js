    ...
    <>
      {!isError && blogs && (
        <div className="w-full relative my-16 py-16 our-blog-bg">
          <h4 className="section-title text-shadow absolute z-10 left-1/2 -translate-x-1/2 -top-6 md:text-5xl md:-top-8">
            {title}
          </h4>
          <div ref={slider} className="blogs-slider px-5">
            {blogs?.map((blog) => (
              <BlogItem
                ...
                id={blog?.id}
                blogItemWidth={blogItemWidth}
              />
            ))}
          </div>
          <button
            onClick={nextBtn}
            className="flex items-center justify-center absolute bottom-4 right-10"
          >
            <svg
