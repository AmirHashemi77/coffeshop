...
const OurBlog = ({title}) => {
    ...
    return (
        <>
            {
                !isError && blogs &&   <div className='w-full relative my-16 py-16 our-blog-bg'>           
                ...
                <div ref={slider} className='blogs-slider px-5'>
                    {blogs?.map((blog)=>(
                        <BlogItem key={blog?.id} title={blog?.title} content={blog?.content} id={blog?.id} blogItemWidth={blogItemWidth}/>
