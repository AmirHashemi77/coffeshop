...
const ProductDetailsSection = ({image,title,subTitle,price,id}) => {
    ...
    return (
        ...
        <div className='flex items-center justify-center w-full px-5 md:w-5/12'>
            <img src={image} alt="product" className='w-full h-96 object-contain object-center' />
        </div>
        ...
        <div className='flex flex-col items-start w-full px-5 flex-1'>
            {/* title section  */}
                <div className='flex flex-col w-full items-start gap-3 pb-6  border-b border-white border-opacity-50 px-3'>
                    {/* title  */}
                    <h1 className='text-white text-3xl font-leiko leading-8 text-left md:text-4xl lg:text-5xl capitalize opacity-80'>{title}</h1>
                    {/* subtitle  */}
                    <h3 className='text-white text-lg font-leiko leading-8 text-left capitalize opacity-40 '>{subTitle}</h3>
